...
# Absolute paths (baseline chunks from BigBird prep)
DATA_DIR    = "/mount/studenten-temp1/users/qbaibisa/party-positioning-code-master/data"
RESULTS_DIR = "/mount/studenten-temp1/users/qbaibisa/party-positioning-code-master/results"
CHUNK_FILE  = os.path.join(DATA_DIR, "chunks_for_manifestos_bigbird.json")
RILE_FILE   = os.path.join(DATA_DIR, "RILE_by_chunk_bigbird.json")
DATASET_TSV = os.path.join(DATA_DIR, "longt5_chunk_training_data_baseline.csv")
...
def train_epoch(epoch_n, data, batch_size, tokeniser, encoder, regression_head, optimiser, max_length):
    ...
    for step in tqdm(range(n_steps), desc=f'Epoch {epoch_n+1}, training', leave=False):
        ...
        tokens = tokeniser(batch_texts, truncation=True, max_length=max_length,
...
def main():
    ...
    idx = list(range(data_train_all.shape[0])); shuffle(idx)
    dev_size = len(idx)//10
    ...
    encoder   = LongT5EncoderModel.from_pretrained(
        model_name,
        cache_dir="/mount/studenten-temp1/users/qbaibisa/hf_cache"
    )
    ...
    n_epochs   = 5
    batch_size = 1
    max_length = 4096   # shorter to reduce instability
    ...
    for epoch_n in range(n_epochs):
        ...
        if dev_r is not None and dev_r > best_dev_r:
            ...
            with open(os.path.join(RESULTS_DIR, "longt5_ovn_baseline_predictions.json"), 'w') as out:
                json.dump({'spearman': test_r, 'preds': test_preds}, out)

    print(f"BEST dev_spearman={best_dev_r:.4f} | test_spearman_at_best={test_r_at_best:.4f}")
