...
def run_epoch(data, tokeniser, encoder, head, batch_size, max_length, train=False, optimiser=None):
    ...
    for step in tqdm(range(n_steps), leave=False):
        ...
        toks = tokeniser(texts, truncation=True, max_length=max_length,
        ...
        with torch.set_grad_enabled(train):
            ...
            out = head(reps)
            if train:
                gold = torch.tensor(data.RILE[lo:hi].values, dtype=torch.float32,
...
def main():
    ...
    results_path = "memformer_results.json"
    ...
    for (country, data_train_all, data_test) in utils.split_chunk_data_loco(all_data):
        ...
        print(f"\nRunning LOCO for {country} | train={len(data_train)}, dev={len(data_dev)}, test={len(data_test)}")
        best_dev, best_test, best_preds = -1.0, -1.0, []

        for ep in range(n_epochs):
            tr_loss, _ = run_epoch(data_train, tokeniser, encoder, head, batch_size, max_length, train=True, optimiser=opt)
            _, dev_preds = run_epoch(data_dev, tokeniser, encoder, head, batch_size, max_length, train=False)
            ...
            if dev_r is not None and dev_r > best_dev:
                best_dev = dev_r
                _, test_preds = run_epoch(data_test, tokeniser, encoder, head, batch_size, max_length, train=False)
                test_r = spearmanr(test_preds, data_test.RILE).correlation
                best_test = test_r
                best_preds = test_preds
                results[country] = {"dev_r": float(best_dev), "test_r": float(best_test)}

        print(f"BEST for {country}: dev_r={best_dev:.4f}, test_r={best_test:.4f}")
        print("Sample predicted RILE scores (first 20):")
        print(best_preds[:20])

    with open(results_path, 'w') as out:
        json.dump(results, out, indent=2)
    print("\nSaved overall results:", results)
