import sys
sys.path.append('/mount/studenten-temp1/users/qbaibisa/thesis-venv/lib64/python3.13/site-packages')
...
DATASET_TSV = os.path.join(DATA_DIR, "memformer_chunk_training_data.csv")
...
def main():
    ...
    # Build Memformer encoder
    encoder = MemorizingTransformer(
        num_tokens=30522,        # roberta-base vocab size
        dim=512,
        depth=6,
        max_seq_len=8192,
    ...
    # Run only on a few countries for credibility
    target_countries = {"Germany", "Finland", "Argentina"}
    ...
    out_path = os.path.join(RESULTS_DIR, "memformer_loco_selected.json")
